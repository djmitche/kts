...
#ifdef KPYM_HACK
extern ALGO_STRING_INFO FAR_BSS algoStringEncrTblClient[];
#else
...
#endif
#ifdef KPYM_HACK
extern ALGO_STRING_INFO FAR_BSS algoStringEncrTblServer[];
#else
...
#ifdef KPYM_HACK
					   algoSize( algoStringEncrTblServer ),
#else
...
#ifdef KPYM_HACK
					   algoSize( algoStringEncrTblClient ),
#else
...
#ifdef KPYM_HACK
	if( sshInfo->packetType == SSH2_MSG_NEWKEYS )
	{
		STREAM stream;
		status = processChannelControlMessage( sessionInfoPtr, &stream );
